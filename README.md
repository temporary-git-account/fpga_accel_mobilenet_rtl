...
docs/images/              architecture, tile, schedule and pipeline SVG diagrams
...
1. Read sections 1–6 before following the layer scheduler.
...
5. Compare proposed optimizations against the same numerical and timing gates.

Generated programming containers and workstation build databases are excluded.
Workstation paths in the supplied reports are normalized; numerical results and
original source notices are retained. No Git repository or commit was created
for this package.

## Contents

- [1. What this hardware implements](#chapter-1)
- [2. Source map: read these blocks in order](#chapter-2)
- [3. Tensor layout and convolution math](#chapter-3)
- [4. Quantization is arithmetic, not just an integer datatype](#chapter-4)
- [5. The 4×8 compute tile](#chapter-5)
- [6. Requantization as an eight-stage circuit](#chapter-6)
- [7. The layer scheduler is the main architecture](#chapter-7)
- [8. From the internal memory port to AXI](#chapter-8)
- [9. AXI-Lite control and layer descriptors](#chapter-9)
- [10. Cancellation and reset behavior](#chapter-10)
- [11. Clock domains and physical implementation](#chapter-11)
- [12. A synthesis bug that RTL simulation did not catch](#chapter-12)
- [13. Verification ladder and what each rung proves](#chapter-13)
- [14. The complete operator inventory](#chapter-14)
- [15. Where to optimize next](#chapter-15)
- [16. Study exercises](#chapter-16)
- [17. Glossary and suggested reading order](#chapter-17)
...
![Production RTL blocks from AXI-Lite control through tile banks, MAC, requantizer, cache and DDR](docs/images/architecture.svg)

*Figure 1. Production blocks. The numerical testbench supplies descriptors and
DDR contents; activation/weight fetch, convolution and output stores are driven
by the RTL.*
...
#### Worked datapath values and validity checks

The two examples below show every intermediate integer value. All
operations use exact integers; arithmetic right shifts round downward before
the explicit remainder test. Both examples use clamp bounds `[-128,127]`.

| Quantity | Positive worked example | Negative halfway example |
|---|---:|---:|
| Accumulator | 125 | −3 |
| Bias | 3 | 0 |
| Multiplier `M` | 1,073,741,824 | 1,073,741,824 |
| Right shift `s` | 2 | 1 |
| Output zero point | −5 | 0 |
| Biased accumulator `v` | 128 | −3 |
| Signed product `v × M` | 137,438,953,472 | −3,221,225,472 |
| High Q31 result `h` | 64 | −1 |
| Shift base `h >> s` | 16 | −1 |
| Mask | 3 | 1 |
| Remainder | 0 | 1 |
| Threshold | 1 | 1 |
| Round increment (`remainder > threshold`) | 0 | 0 |
| Scaled value | 16 | −1 |
| After output zero point | 11 | −1 |
| Clamped int8 result | **11** | **−1** |

To calculate another case, apply section 4.3 in this same order. Accumulator
and bias must each fit signed int32, and their sum must also fit. `M` must be
1…2,147,483,647; `s` must be 0…31. The zero point and ordered clamp bounds must
fit signed int8. The calculator's input validation and the RTL interface have
different boundaries: shifts and byte fields already have fixed widths at the
RTL port, while bias overflow, an invalid multiplier or reversed limits produce
an arithmetic error. The layer descriptor checks wider parameter values before
narrowing them to those ports.

Saturation is separate from rounding: `acc=1000, bias=0, M=2³⁰, s=0, Zout=0`
produces a scaled value of 500 and clamps to 127 with full int8 bounds; changing
the upper activation bound to 6 clamps it to 6. For `acc=−1000`, the scaled
value is −500 and clamps to −128. Overflowing `2,147,483,647 + 1` is an error,
not saturation to a valid output.
...
![A 4 by 8 MAC array showing operand sharing and a three-pixel, five-channel tail](docs/images/compute-tile.svg)

*Figure 2. Green cells are useful lanes; grey cells are masked. Text labels
also identify each state so the illustration does not depend on color.*

For any selected lane `(p,c)`, regular convolution uses `activation[p]` and
`weight[c]`; depthwise uses `activation[p,c]` and `weight[c]`. For example,
lane `(2,4)` updates `sum[2,4]` from those operands. A lane writes only when
`p < valid_pixels` and `c < valid_channels`. Lane `(3,7)` is masked in the
3×5 example. These are positions within a tile, not global tensor coordinates.

| Valid pixels | Valid channels | Useful lanes / 32 | Tile occupancy |
|---:|---:|---:|---:|
| 4 | 8 | 32 | 100% |
| 3 | 5 | 15 | 46.875% |
| 4 | 2 | 8 | 25% |
...
Valid pixel counts range from 1 to 4 and valid channel counts from 1 to 8.
`occupancy = valid_pixels × valid_channels / 32`. This measures useful lanes
inside a tile, independently of how many clock cycles are spent feeding it.
...
![Eight requantizer stages, from bias addition through partial products, signed rounding and clamp](docs/images/requantization-pipeline.svg)

*Figure 3. One pipeline handles the 32 sums in a tile. Arrows show stage order;
all stages hold together when output backpressure prevents advancement.*
...
![Eight phases of the repeated layer schedule with the RTL state groups](docs/images/layer-schedule.svg)

*Figure 4. Loading, computing and writing are largely serialized; MREQ/MWAIT
are shared substeps of the memory phases.*
...
#### Arithmetic ceiling and assumed feed duty

Evaluate the effect of an assumed feed duty directly:

```text
rate_MAC_per_second = 32 × frequency_MHz × 1,000,000 / cycles_per_term × duty_percent / 100
arithmetic_seconds = 567,718,400 / rate_MAC_per_second
```

Here duty is the assumed fraction of time spent running the selected feed
schedule. Assume all 32 lanes are useful during feed. It is an input to a
thought experiment, not measured utilization. Frequency must be positive,
cycles per term is 1 or 2, and duty must be greater than zero through 100%.

| Frequency (MHz) | Cycles / term | Assumed feed duty | Useful rate (GMAC/s) | Arithmetic estimate (ms/image) |
|---:|---:|---:|---:|---:|
| 166.667 | 2 | 100% | 2.667 | 212.9 |
| 166.667 | 2 | 50% | 1.333 | 425.8 |
| 166.667 | 2 | 10% | 0.267 | 2128.9 |
| 166.667 | 1 | 100% | 5.333 | 106.4 |
| 200 | 2 | 100% | 3.200 | 177.4 |

One-cycle feeding is hypothetical for the current scheduler. The 200 MHz row
is arithmetic only: the tested 200 MHz physical candidate failed timing. These
estimates do not simulate DDR queueing, quantizer drain, tail tiles, control
overhead or a new timing result. Use measured state occupancy and lane masks
before predicting an actual implementation speedup.
